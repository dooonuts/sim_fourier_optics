...
%     S_tA = D_tA;
    ...
    for j=1:length(phi_)
        ...
        S_tA(:,:,3*(i-1)+j) = circshift(D_tA(:,:,j),-kshift);
    end    
...
figure; imagesc(abs(1+log(S_tA_sum)));
...
% 
% for i=1:length(phi_)
%     maximum = max(max(D_tA(:,:,i))); [ind1,ind2] = find(D_tA(:,:,i) == maximum);
%     kshift = [round(ind1-pixels/2), round(ind2-pixels/2)];
%     S_tA(:,:,i) = circshift(D_tA(:,:,i),-kshift);
% end    
% % maximum = max(max(D_tAp2)); [ind1,ind2] = find(D_tAp2 == maximum);
% % kshift = [round(ind1-pixels/2), round(ind2-pixels/2)];
% % S_tAp2 = circshift(D_tAp2,-kshift);
